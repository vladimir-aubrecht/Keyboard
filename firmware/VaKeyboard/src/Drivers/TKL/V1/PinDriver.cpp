...
uint8_t PinDriver::readPin(uint8_t pinNumber)
{
	...
	{
		uint32_t mask = (1 << (15 - pinNumber));
		return ((cache >> 16) & mask) >> (15 - pinNumber );
	}
	else
	{
		uint32_t mask = (1 << (31 - pinNumber));
		return ((cache >> 16) & mask) >> (31 - pinNumber);
	}
}
